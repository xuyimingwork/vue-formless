...
const notes: Record<DemoId, { baseline: string; formless: string; compare: string }> = {
  basic: {
    ...
    formless:
      '共用 User 控件表；模板只摆控件。FormView 自带 Form；`fl:layout` 用工厂密度 2/16，备注 `layout-item:span="max"`。',
    compare: '同一套字段，对照模板密度。',
  },
  filter: {
    baseline: '四格筛选，手写 Col span=6。',
    formless: '同一套 User.*，只点四个；placeholder / clearable 写在标签上。`fl:layout` + `:layout:column="4"`。',
  ...
  mixed: {
    ...
    formless:
      '同一套 User.*；外层 FormView 包 Form；内层只写 `fl:layout` + `:layout:*` 换密度（`fl:form` auto 关、v-model inherit）。',
    compare: '同一套分组布局，对照托管与逃逸。',
  },
  range: {
    baseline: '三列混排：行程日期 span 16 旁塞一格；开始+结束同样两列再塞一格。',
    formless:
      "Range One 一格 `layout-item:span=\"16\"`；Range Two control `field: embed`，页上 `:fl:field=\"'wrap-embed'\"` 分组壳；备注是临场 FormField（`:fl:prop` / `layout-item:span`）。",
