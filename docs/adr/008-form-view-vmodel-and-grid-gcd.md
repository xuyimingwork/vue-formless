...
```vue
<FormView ref="formRef" v-model="form" label-width="96px" fl:layout>
  <User.Name />
  <User.DateRange />
</FormView>
```
...
格宽 `:col:span`（省略=`1x`，`'Nx'` / `'max'` / 绝对 1–24）；对齐 `:col:place`（`auto` / `start` / `end`）。空白格是 fragment 里的 Col，不用 `offset`。
