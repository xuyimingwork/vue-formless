...
- [`create-form-fields.ts`](../../packages/vue-formless/src/create-form-fields.ts)：域表工厂，每项走 `createFormField`（`FieldSchemaInput` / `FieldFactoryInput` 类型已并入 `field-schema.ts`）
